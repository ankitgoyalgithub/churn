...
class HealthAssessment:
    def __init__(self, ID, target, churn_date, snapshot_date, target_window=8,metrics_col=[]):
        ...
        self.score_col.extend([self.ID, self.snapshot_date])
    ...
    def preprocess_data(self, outcome, score_history,company):
        ...
        # preprocess history
        score_history.rename(columns={'Scorecard Id': 'scorecard id', 'Company Id': 'account_id', 'Created Date': 'created_date'}, inplace=True)
    ...
    def create_account_date_set(self,data,start,end,intervals=52):
        ...
        week_ids = pd.DataFrame({self.ID:sorted((list(data[self.ID]))*intervals), 'score_week':date_list*data[self.ID].nunique()})
    ...
    def run_health_assessment(self,history_status):
        ...
        sample_set=self.create_account_date_set(etdata_sample,str(history_status[self.snapshot_date].min().date()),str(history_status[self.snapshot_date].max().date()))
        ...
        model_record=model_record[model_record.week_advance.apply(lambda x:True if abs(x)> (self.target_window-1) else False)].copy()
        return model_record